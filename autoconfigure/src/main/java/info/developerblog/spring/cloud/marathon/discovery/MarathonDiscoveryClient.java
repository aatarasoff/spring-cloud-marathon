...
public class MarathonDiscoveryClient implements DiscoveryClient {
        ...
        try {
        	...
        	if (!allServices(serviceId)) {
        		...
        		if (instances.isEmpty()) {
        			instances.addAll(getInstances(new SearchServiceQuery(serviceId)));
        		}
        	} else {
    ...
    private List<ServiceInstance> getInstances() throws MarathonException {
    	return getInstances((SearchServiceQuery)null);
    }
    
    private List<ServiceInstance> getInstances(SearchServiceQuery query) throws MarathonException {
    	List<ServiceInstance> instances = new ArrayList<>();
    	
        GetAppsResponse appsResponse = query == null ? client.getApps() : client.getApps(query.queryMap);

        if (appsResponse != null && appsResponse.getApps() != null) {
        	List<App> apps = appsResponse.getApps();
        	
        	log.debug("Discovered {} service{}{}", apps.size(), apps.size() == 1 ? "" : "s", query == null ? "" : String.format(" with ids that contain [%s]", query.serviceId));
    ...
    private static class SearchServiceQuery {
    	private final String serviceId;
    	private Map<String,String> queryMap = new HashMap<>();
    	
    	private SearchServiceQuery(String serviceId) {
    		this.serviceId = serviceId;
    		this.queryMap.put("id", ServiceIdConverter.convertToMarathonId(serviceId));
    	}
    }
}
